...
此功能通过顶级窗口注册 `RegisterPowerSettingNotification` 订阅 `GUID_CONSOLE_DISPLAY_STATE`，完全跟随 Windows 自身的显示器电源状态广播（不做自己的空闲计时）。因此当你在看视频、听音乐或有其他应用阻止显示器关闭时，灯不会被误关；只有 Windows 真正决定关闭显示器时，灯才会跟着关。程序会智能记忆关灯前的开关状态，显示器唤醒时自动恢复。
